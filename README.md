...
“质量警告”不表示压缩失败：它表示输出已通过结构、编码、音频、分辨率和时长验证，但质量检查仍有警告，Maximum Detail 输出会被保留以供人工复核。Maximum Detail 重试后的质量检查报错也会记录为“质量检查失败”，并保留该结构有效输出供人工复核。只有重试编码失败或取消时才会恢复先前的 Best Detail 输出；恢复失败才会标记失败。
