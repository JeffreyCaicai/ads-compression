...
Before retrying, the successful Best Detail output is moved to a hidden same-directory `.quality-backup` path. Retry encode failure, quality-check cancellation, cancellation before/during retry, or an unexpected retry exception restores the Best file and its plan, targets and audit fields. A failed restore is an actual failure. If Maximum Detail is retained, the backup is deleted. Non-Auto Detail two-pass modes retain their existing behavior and do not run this quality loop.
